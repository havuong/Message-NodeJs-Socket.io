...
(function(document, window, $) {
    ...
    var default_options = {
        template: `
            <div class="emoji-box"><tabs/><filters/></div>
            <div>
                <div class="text-input"><editor/></div>
                <div class="show-emoji-box"><i class="icon-emoticon-smile"></i></div>
            </div>
        `,
    ...
    function init(self, source, options) {
        ...
        app = $('<div/>', {
            ...
            role: "application"
        }).addClass("emojionearea").html(app);
        ...
        filtersArrowLeft = $('<i class="emojionearea-filter-arrow-left"/>', {
            role: "button"
        }).appendTo(filters);
        filtersArrowRight = $('<i class="emojionearea-filter-arrow-right"/>', {
            role: "button"
        }).appendTo(filters);
        ...
        ncdai(filters.find('.emojionearea-filter[data-filter=people]'));

        var showEmojiBox = app.find('.show-emoji-box');

        attach(self, [filters, tabs, showEmojiBox], {
            mousedown: "area.mousedown"
        }, editor);
        attach(self, editor, {
            paste: "editor.paste"
        }, editor);
        attach(self, editor, ["focus", "blur"], function() {
            return !!stayFocused ? false : editor;
        });
        attach(self, [editor, filters, tabs], ["mousedown", "mouseup", "click", "keyup", "keydown", "keypress"], editor);
        attach(self, filters.find(".emojionearea-filter"), {
            click: "filter.click"
        });
        attach(self, filters.find(".emojionearea-filter"), {
            click: "filter.click"
        });
        attach(self, showEmojiBox, {
            click: "showEmojiBox.click"
        });
        attach(self, filtersArrowLeft, {
            click: "arrowLeft.click",
            mousedown: "arrowLeft.mousedown",
            mouseup: "arrowLeft.mouseup"
        });
        attach(self, filtersArrowRight, {
            click: "arrowRight.click",
            mousedown: "arrowRight.mousedown",
            mouseup: "arrowRight.mouseup"
        });
        ...
        .on("@showEmojiBox.click", function() {
            app.find('.emoji-box').toggle();
        })
